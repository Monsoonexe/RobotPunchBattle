...
def main():
    ...
    while gameIsPlaying:
        ...
        playerBotHealth = playerBot.getHp
        print(playerBotHealth)
        compBotHealth = compBot.getHp
        while playerBotHealth > 0 and compBotHealth > 0: #while both robots alive
            if turn == "PLAYER":
...
def heal(atkr, move): #heal mechanic
    ...
    print(atkr.getName, "is attempting to put itself back together.") #dialogue
    #resolve miss
    if checkMiss == 'y':
...
def damage(atkr, dfndr, move): #effect)
    ...
    if checkMiss == "y": #attack missed dialogue
        ...
        print(atkr.getName, "used" + move['Name'] + "!") #dialogue
...
##        noModDamage = int(((atk.attack / dfs.defense) * pwr + 2) * critPower)#
            #print test
        dmg = int((((atkr.getAttack / dfndr.getDefense) * move['Pwr'] + 2) * critPower) * mod)
...
def compChooseAttack(comp):#what will AI do?
    if comp.getHp >= .9 * comp.MAXHP:  #if comp is high health
        choice = 'robot_slap' #ROBOT SLAP
    elif comp.getHp >= .7 * comp.MAXHP: # if comp medium high health
        choiceList = ('robot_punch', 'robot_slap', 'electro_shock')
        choice = random.choice(choiceList)
    elif comp.getHp >= .5 * comp.MAXHP:#if comp is medium health
        choiceList = ('robot_punch', 'robot_slap', 'electro_shock', 'heal')
        choice = random.choice(choiceList) #any move
    elif comp.getHp >= .1 * comp.MAXHP: # if near death
