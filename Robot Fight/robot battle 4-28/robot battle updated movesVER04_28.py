...
###why is "xBox's current HP" printing twice?
...
def heal(atkr, move): #heal mechanic
    #accuracy check
    checkMiss = checkAccuracy(move['moveAcc']) #returns y or n
    #crit check
    critResult = checkCrit(move['moveCrit']) #returns either y or n
    ...
    if checkMiss == 'y':
        ...
        healAmount = int((atkr.MAXHP * (move['movePwr']/100) * criticalHeal) * mod)
...
##        noModDamage = int(((atk.attack / dfs.defense) * pwr + 2) * critPower)#
            #print test
        dmg = int((((atkr.attack / dfndr.defense) * move['movePwr'] + 2) * critPower) * mod)
...
def activateAttack(atkr, dfndr, move): # finds the move and calls damage
    if move["moveKind"] == 'atk': # move Kind is an attack
        damage(atkr, dfndr, move) # run damage function
    elif move["moveKind"] == 'heal': #move Kind is a restore move
